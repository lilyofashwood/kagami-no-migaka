...
document.querySelector('section .hint').textContent='Choose the structural example, the recovered Nekomata grid, or the supplied Kitsune artifact. Each follows the same two verified paths. Turn the grid and follow the second voice. Coordinates are [row, column], beginning at zero.';
...
const note=document.createElement('p');note.className='hint';note.textContent='The selected title is Kagami-no-Migaka. The former title かのとこよ · 歌ノ常世 lives in the README poetry beside よことのか · 夜言ノ香: a pair of answering readings. Kasaneuta remains the historical method. Turn these earlier naming cards to reveal the second reading hidden in their exact kana.';cards.append(note);
...
$('compose').onclick=()=>modelAction(async()=>{
  ...
  const result=assembleCandidate(candidate,current.paths);put({...result.document,provenance:'New model-composed grid using the Kasaneuta method.'});verify();show({semanticStatus:result.semanticStatus,interpretations:result.interpretations},'model-result');
});
$('interpret').onclick=()=>modelAction(async()=>{
  ...
  show({status:'model interpretation beside the exact grid readings',result},'model-result');
});
