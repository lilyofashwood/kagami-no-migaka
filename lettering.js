...
// Interface-only adapter. Never apply this to grid cells, byte envelopes,
...
export function letterInterface(root=document){
  ...
  const body=[...root.querySelectorAll('h1,header p,section p,details p,label,button,option,footer,a')];
  ...
  for(const [element,register] of entries){
    if(element.closest('pre,code,textarea,input,[data-literal]'))continue;
    const label=[...element.childNodes].filter(n=>n.nodeType===3).map(n=>n.nodeValue).join(' ').trim();
    if(label&&element.matches('h2,h3,summary,button,a'))element.setAttribute('aria-label',label.normalize('NFKC'));
    ...
    for(const node of nodes){
      if(node.parentElement.closest('pre,code,textarea,input,[data-literal]'))continue;
